 ...
 **Correction:** On va classer par origine pour lire la réponse. Mais bon, si le moyen le plus efficace pour comparer deux croissances (variable que l'on représente sur l'axe des ordonnées) est sa position sur l'axe des abscisses c'est qu'il y a un problème !!!!
...
NUTS2_year %>%
  ...
  ggplot(aes(x=id_anc, y=croissance)) +
  geom_col() +
  geom_col(data=.%>% group_by(), fill='darkblue') +
  geom_text(
    aes(
      label = id_anc,
      y     = ifelse(croissance>0, -10000, 10000),
